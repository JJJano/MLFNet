...
def rotate_instance(img,obj_depth,obj_normal, mask, hcoords, sym_cor, rot_ang_min, rot_ang_max):
    ...
    obj_depth = cv2.warpAffine(obj_depth, R, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0)
    obj_normal = cv2.warpAffine(obj_normal, R, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT,
                               borderValue=0)
    last_row = np.asarray([[0, 0, 1]], dtype=np.float32)
    R = np.concatenate([R, last_row], axis=0).transpose()
    ...
    hcoords = hcoords[:, :2]
    return img, mask, hcoords, sym_cor,obj_depth,obj_normal


def crop_resize_instance_v1(img,obj_depth,obj_normal, mask, hcoords, sym_cor, imheight, imwidth,
    ...
    img, mask, hcoords, sym_cor,obj_depth,obj_normal = crop_or_padding_to_fixed_size_instance(
        img,obj_depth,obj_normal, mask, hcoords, sym_cor, target_height, target_width, overlap_ratio)

    img = cv2.resize(img, (imwidth, imheight), interpolation=cv2.INTER_LINEAR)
    obj_depth = cv2.resize(obj_depth, (imwidth, imheight), interpolation=cv2.INTER_NEAREST)
    obj_normal = cv2.resize(obj_normal, (imwidth, imheight), interpolation=cv2.INTER_NEAREST)
...
def crop_or_padding_to_fixed_size_instance(img,obj_depth,obj_normal, mask, hcoords, sym_cor, th, tw,
                                           overlap_ratio=0.5):
    h, w, _ = img.shape
    hs, ws = np.nonzero(mask)
    ...
    obj_depth = obj_depth[hbeg:hend, wbeg:wend]
    obj_normal = obj_normal[hbeg:hend, wbeg:wend]
    ...
    if hpad or wpad:
        ...
        new_obj_depth = np.zeros([th, tw, 3], dtype=obj_depth.dtype)
        new_obj_normal = np.zeros([th, tw, 3], dtype=obj_normal.dtype)
        ...
        new_obj_depth[hbeg:hbeg + nh, wbeg:wbeg + nw] = obj_depth
        new_obj_normal[hbeg:hbeg + nh, wbeg:wbeg + nw] = obj_normal
        new_mask[hbeg:hbeg + nh, wbeg:wbeg + nw] = mask
        new_sym_cor[hbeg:hbeg + nh, wbeg:wbeg + nw] = sym_cor
        hcoords[:, 0] += wbeg * hcoords[:, 2]
        hcoords[:, 1] += hbeg * hcoords[:, 2]

        img, mask, sym_cor,obj_depth,obj_normal = new_img, new_mask, new_sym_cor,new_obj_depth,new_obj_normal

    return img, mask, hcoords, sym_cor,obj_depth,obj_normal


def crop_or_padding_to_fixed_size(img,obj_depth,obj_normal, mask, sym_cor, th, tw):
    h, w, _ = img.shape
    hpad, wpad = th >= h, tw >= w
    ...
    return img, mask, sym_cor,obj_depth,obj_normal
