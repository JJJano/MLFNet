        ...
        for object_name in self.object_names:
            if local_idx < self.lengths[object_name]:
                ...
                obj_depth_name = os.path.join('data/obj_depth_normal',
                                          object_name, 'occ_depth', 'obj_{}.npy.npz'.format(str(img_id).zfill(6)))
                obj_depth =np.float32( np.load(obj_depth_name)['arr_0'])/100.

                obj_normal_name = os.path.join('data/obj_depth_normal',
                                              object_name, 'occ_normals',
                                              'obj_{}.npy.npz'.format(str(img_id).zfill(6)))
                obj_normal = np.float32(np.load(obj_normal_name)['arr_0'])
