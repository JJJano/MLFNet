...
class CoreTrainer(object):
    def __init__(self, model, optimizer, train_loader, test_loader, val_loader, args):
    ...
    def train(self, epoch):
        ...
        dn_mask_loss_record = AverageMeter()
        obj_depth_loss_record = AverageMeter()
        obj_normal_loss_record = AverageMeter()
        ...
        for i_batch, batch in enumerate(self.train_loader):
            ...
            if cuda:
                ...
                batch['obj_depth'] = batch['obj_depth'].cuda()
                batch['obj_normal'] = batch['obj_normal'].cuda()
            (sym_cor_pred, mask_pred, pts2d_map_pred, graph_pred, sym_cor_loss, mask_loss, pts2d_loss, graph_loss,
             dn_mask_loss, obj_depth_loss, obj_normal_loss, _, _) = self.model(
                batch['image'], batch['sym_cor'], batch['mask'], batch['pts2d_map'], batch['graph'], batch['obj_depth'],
                batch['obj_normal'], epoch)
            ...
            dn_mask_loss = dn_mask_loss.mean()
            obj_depth_loss = obj_depth_loss.mean()
            obj_normal_loss = obj_normal_loss.mean()
            current_loss = \
                self.args.lambda_sym_cor * sym_cor_loss + self.args.lambda_mask * mask_loss + \
                self.args.lambda_pts2d * pts2d_loss + self.args.lambda_graph * graph_loss + \
                self.args.lambda_x_mask * dn_mask_loss + self.args.lambda_obj_depth * obj_depth_loss + \
                self.args.lambda_obj_normal * obj_normal_loss
            # Step optimizer
            ...
            dn_mask_loss_record.update(dn_mask_loss.detach().cpu().numpy(), len(batch['image']))
            obj_depth_loss_record.update(obj_depth_loss.detach().cpu().numpy(), len(batch['image']))
            obj_normal_loss_record.update(obj_normal_loss.detach().cpu().numpy(), len(batch['image']))
            ...
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Mask: {mask.val:.6f} ({mask.avg:.6f})\t'
                  'Pts: {pts.val:.6f} ({pts.avg:.6f})\t'
                  'Graph: {graph.val:.4f} ({graph.avg:.4f})\t'
                  'dn_mask: {dn_mask.val:.6f} ({dn_mask.avg:.6f})\t'
                  'obj_depth: {obj_depth.val:.6f} ({obj_depth.avg:.6f})\t'
                  'obj_normal: {obj_normal.val:.6f} ({obj_normal.avg:.6f})\t'
                  'Total: {total.val:.4f} ({total.avg:.4f})'.format(epoch, i_batch, len(self.train_loader),
                                                                    ...
                                                                    graph=graph_loss_record,
                                                                    dn_mask=dn_mask_loss_record,
                                                                    obj_depth=obj_depth_loss_record,
                                                                    obj_normal=obj_normal_loss_record,
                                                                    total=total_loss_record))
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for i_batch, batch in enumerate(data_loader):
                if cuda:
                    ...
                    batch['obj_normal'] = batch['obj_normal'].cuda()
                (sym_cor_pred, mask_pred, pts2d_map_pred, graph_pred, sym_cor_loss, mask_loss, pts2d_loss, graph_loss,
                 dn_mask_loss, obj_depth_loss, obj_normal_loss, _, _) = self.model(batch['image'], batch['sym_cor'],
                                                                                   batch['mask'], batch['pts2d_map'],
                                                                                   batch['graph'],
                                                                                   batch['obj_depth'],
                                                                                   batch['obj_normal'], epoch)
                ...
                obj_normal_loss = obj_normal_loss.mean()
                current_loss = \
                    self.args.lambda_sym_cor * sym_cor_loss + \
        ...
        with open('output/linemod/loss_{}.txt'.format(self.args.object_name), 'a') as file:
            file.write(str(epoch))
            file.write(": ")
            file.write(str(loss_record.avg))
            file.write("\n")
            file.write('Sym: ({sym.avg:.4f}) '
                       'Mask: ({mask.avg:.6f}) '
                       'Pts:  ({pts.avg:.6f}) '
                       'Graph: ({graph.avg:.4f}) '
                       'dn_mask: ({dn_mask.avg:.6f}) '
                       'obj_depth:  ({obj_depth.avg:.6f}) '
                       'obj_normal: ({obj_normal.avg:.6f})'.format(sym=sym_cor_loss_record, mask=mask_loss_record,
                                                                   pts=pts2d_loss_record, graph=graph_loss_record,
                                                                   dn_mask=dn_mask_loss_record,
                                                                   obj_depth=obj_depth_loss_record,
                                                                   obj_normal=obj_normal_loss_record))
            file.write("\n")
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            for i_batch, batch in enumerate(data_loader):
                if cuda:
                    ...
                    batch['obj_normal'] = batch['obj_normal'].cuda()
                (sym_cor_pred, mask_pred, pts2d_map_pred, graph_pred, sym_cor_loss, mask_loss, pts2d_loss, graph_loss,
                 dn_mask_loss, obj_depth_loss, obj_normal_loss, _, _) = \
                    self.model(batch['image'], batch['sym_cor'], batch['mask'], batch['pts2d_map'], batch['graph'],
                               batch['obj_depth'], batch['obj_normal'], epoch)
                mask_pred[mask_pred > 0.5] = 1.
                mask_pred[mask_pred <= 0.5] = 0.
        ...
        with open('output/linemod/val_loss_{}.txt'.format(self.args.object_name), 'a') as file:
            file.write(str(epoch))
            file.write(": ")
            file.write(str(loss_record.avg))
            file.write("\n")
    ...
    def fill_intermediate_predictions(self, regressor, predictions, K_inv, pts3d, pts2d_pred_loc, pts2d_pred_var,
                                      graph_pred, sym_cor_pred, mask_pred, normal_gt):
    ...
    def regress_pose(self, regressor, predictions, pr_para, pi_para, K_inv, pts3d, pts2d_pred_loc, pts2d_pred_var,
                     graph_pred, sym_cor_pred, mask_pred, normal_gt):
        if mask_pred.sum() == 0:
    ...
    def generate_data(self, val_loader, val_size=50, epoch=None):
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            for i_batch, batch in enumerate(val_loader):
                ...
                sym_cor_pred, mask_pred, pts2d_map_pred, graph_pred, sym_cor_loss, mask_loss, pts2d_loss, graph_loss, _, _, _, _, _ = \
                    self.model(batch['image'], batch['sym_cor'], batch['mask'], batch['pts2d_map'], batch['graph'],
                               batch['obj_depth'], batch['obj_normal'], epoch)
            ...
            for i_batch, batch in enumerate(self.test_loader):
                ...
                if cuda:
                    ...
                    batch['obj_normal'] = batch['obj_normal'].cuda()
                (sym_cor_pred, mask_pred, pts2d_map_pred, graph_pred, sym_cor_loss, mask_loss, pts2d_loss, graph_loss,
                 _, _, _, depth_pred, normal_pred) = \
                    self.model(batch['image'], batch['sym_cor'], batch['mask'], batch['pts2d_map'], batch['graph'],
                               batch['obj_depth'], batch['obj_normal'], epoch)
                mask_pred[mask_pred > 0.5] = 1.
                mask_pred[mask_pred <= 0.5] = 0.
                pts2d_pred_loc, pts2d_pred_var = self.vote_keypoints(pts2d_map_pred, mask_pred)
                pts2d_loc, pts2d_var = self.vote_keypoints(batch['pts2d_map'], batch['mask'])
                mask_pred = mask_pred.detach().cpu().numpy()
                for i in range(batch['image'].shape[0]):
                    R = batch['R'].numpy()
                    t = batch['t'].numpy()
                    ...
                    test_set['local_idx'].append(batch['local_idx'].numpy()[i])
